...
//aérer le code ?
//remplacer tous les "index" par "indice" pour françiser un peu la chose pour le rapport (qu'on ne doit pas faire au final)
//de même pour debounce (anti rebond)
//Les maquettes écrans, on s'en fout un peu, non ?
...
void loop() {
    ...
    if (tableau_temps_anemometre[!index_tableau_anemometre] - tableau_temps_anemometre[index_tableau_anemometre] < 1000 && tableau_temps_anemometre[1]) {
        ...
        vitesse_vent = pre_calcul_vitesse_vent * (1.0 / ( float(tableau_temps_anemometre[!index_tableau_anemometre] - tableau_temps_anemometre[index_tableau_anemometre] ) / 0.002 ) );
    }//c'est censé fonctionner en faisant *0.002 (si ça marche plus, go refaire /500) 
    else {
        vitesse_vent = 0;
    }
    /* Calcul vitesse du vent :
       ...
       *
       * On la multiplie par 2 pour avoir le temps pris pour faire un tour [en ms].
       * On la divise par 1000 pour avoir le temps pris pour faire un tour [en s].
       *
       * (On simplifie dans le calcul : 2/1000 = 0.002. On multiplie le temps pris pour faire
    ...
    /* Selon la fiche technique de l'anémomètre, s'il y a un contact par seconde entre les deux broches, le vent souffle à 2.4km/h
       ...
       * 
       * En fait, la fiche technique, c'est de la merde.
       *
     */
